...
bot = telebot.TeleBot(settings.PSY20_INTENSIVE_TOKEN, threaded=False)
text = get_start_text()
...
def start_message(message):
    ...
    History.objects.create(user=obj, tgbot=botPsy20intensive, message='запуск или перезапуск бота', mtype='start')
...
def text_message(message):
    ...
    History.objects.create(user=obj, tgbot=botPsy20intensive, message=message.text, mtype='text')
    reply = ''
...
def handle_photo(message):
    ...
    History.objects.create(user=obj, tgbot=botPsy20intensive, message=f"{short_file_path}", mtype='photo')
    
    phone_exist = check_phone_in_last_messages(user_id, botPsy20intensive)
